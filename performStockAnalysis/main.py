...
def select_kode_to_model(n_kode_saham_limit: int = 0):
    ...
    try:
        if n_kode_saham_limit == 0:
            ...
            logging.info("Loaded the full list of stocks from 'Daftar Saham.xlsx'.")
        else:
        ...
        logging.error("The file 'performStockAnalysis/Daftar Saham.xlsx' was not found. Aborting.")
        return np.array([])
...
def save_developed_model(model, kode: str, model_type: str):
    ...
    filename = f'developedModels/{kode}-{model_type}-{developed_date}.pkl'
...
def develop_models_for_selected_kode(selected_kode: list):
    ...
    Returns:
        tuple: A tuple of two DataFrames:
               - (all_model_performances_10_days, all_model_performances_15_days)
    """
